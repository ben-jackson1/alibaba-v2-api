"""Configuration management for Alibaba CLI."""
...
class Config:
    ...
    def from_env(cls, **overrides: str | bool | None) -> "Config":
        ...
        """
        app_key = overrides.get(
            "app_key", os.getenv("ALIBABA_APP_KEY", "")
        ) or overrides.get("app_key", "")
        ...
        if not app_key:
            raise ValueError(
                "ALIBABA_APP_KEY is required. "
                "Set it via environment variable or --app-key flag."
            )
        if not app_secret:
            raise ValueError(
                "ALIBABA_APP_SECRET is required. "
                "Set it via environment variable or --app-secret flag."
            )

        # Parse use_sandbox - can be string "true"/"false" or boolean
        use_sandbox_raw = overrides.get(
        ...
        return cls(
            ...
            timeout=int(
                overrides.get("timeout", os.getenv("ALIBABA_TIMEOUT", "30"))
            ),
        )
